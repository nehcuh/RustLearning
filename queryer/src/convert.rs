...
use polars::prelude::*;
use polars_lazy::dsl::{col, lit, Expr, Operator};
use sqlparser::ast::{
    ...
    fn try_from(expr: Expression) -> Result<Self, Self::Error> {
        match *expr.0 {
            SqlExpr::BinaryOp { left, op, right } => {
                ...
                let op: Operator = Operation(op).try_into()?;
                Ok(Expr::BinaryExpr {
                    left: Arc::new(left),
                    op,
                    right: Arc::new(right),
            ...
            SqlExpr::Value(v) => {
                let lit_value: LiteralValue = Value(v).try_into()?;
                Ok(lit(lit_value))
            }
            SqlExpr::IsNull(expr) => {
                let expr: Expr = Expression(expr).try_into()?;
                Ok(expr.is_null())
            }
...
/// 将 SqlParser 的 BinaryOperator 转换为 DataFrame 的 Operator
...
/// 将 SqlParser 的 SelectItem 转换为 DataFrame 中的 Expr
impl<'a> TryFrom<Projection<'a>> for Expr {
    type Error = anyhow::Error;

    fn try_from(p: Projection<'a>) -> Result<Self, Self::Error> {
        match p.0 {
            SelectItem::UnnamedExpr(SqlExpr::Identifier(id)) => Ok(col(&id.value)),
            SelectItem::ExprWithAlias {
                expr: SqlExpr::Identifier(id),
                alias,
            } => Ok(col(&id.value).alias(&alias.value)),
...
impl<'a> TryFrom<Source<'a>> for &'a str {
    ...
    fn try_from(source: Source<'a>) -> Result<Self, Self::Error> {
        ...
        if !table.joins.is_empty() {
            return Err(anyhow!("We donnot support joint data source at the moment"));
        }
...
/// 把 SqlParser 的 value 转换为 DataFrame 支持的 LiteralValue
...
mod tests {
    ...
    fn parse_sql_works() {
        ...
        let statement: &Statement =
            &Parser::parse_sql(&HcDialect::default(), sql.as_ref()).unwrap()[0];
